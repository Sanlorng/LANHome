...
data class RestResult<T>(val code : Int = -1,
                         var message : String = "",
                         var data : T? = null) {
    ...
    companion object {
        ...
        fun <T>success(data : T) = RestResult<T>(ResultCode.SUCCESS.code,ResultCode.SUCCESS.message).apply {
            this.data = data
        }
        fun <T>failed(message: String = ResultCode.FAILED.message) = RestResult<T>(ResultCode.FAILED.code,message)
